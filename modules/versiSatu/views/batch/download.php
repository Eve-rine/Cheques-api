...
<div style="text-align: center;">Cheques batch <?= ChequesPrint::costBatch($id); ?> generated at <?= date('d-M-Y') ?>.</div>
 <div style="border-top: 2px solid black; border-collapse: collapse; ">
  ...
  <?php foreach(Cheques::find()->where(['batch_id' => $id])->orderBy(['batch_id' => SORT_ASC])->all() as $batch){?>
  <tr>
    ...
    <td style="border: 1px solid black; border-collapse: collapse"><?= date('d M, Y', strtotime($batch->pay_date)); ?></td>
    ...
    <td style="border: 1px solid black; border-collapse: collapse"><?= date('d M, Y',$batch->created_at); ?></td>
